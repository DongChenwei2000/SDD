...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        sim, prob = eval_train_gmm(args, model, train_dataloader, train_length, epoch)
        clean = []
        noisy = []
        ...
        logger.info("Num noisy = %d",len(noisy))

        dataloader_split = prepare_dataloader(args, args.dataset_root, preprocess, logger, 'train', split=clean)
        split_dataloader, split_train_length = dataloader_split['train']

        # NPC training
        train(args, model, siamese_model, split_dataloader, split_train_length, epoch, scheduler, siamese_scheduler, optimizer, siamese_optimizer, loss, prob)
...
def train(args, model, siamese_model, train_dataloader, train_length, epoch, scheduler, siamese_scheduler, optimizer, siamese_optimizer, loss, prob):
    ...
    for idx, batch in enumerate(train_dataloader) :
        ...
        images, texts, _, img_ids, mbankI_imgs, mbankI_txts, mbankT_txts, mbankT_imgs, ori_idx = batch
        ...
        with torch.no_grad():
            ...
            with autocast():
                ...
                ground_truth = torch.arange(len(mbankI_imgs),dtype=torch.long).cuda()
                mbankI_i2t_loss = loss(mbankI_i2t_sim, ground_truth)
                mbankI_t2i_loss = loss(mbankI_t2i_sim, ground_truth)
                mbankI_ori_loss = (mbankI_i2t_loss + mbankI_t2i_loss)/2

                mbankT_i2t_loss = loss(mbankT_i2t_sim, ground_truth)
                mbankT_t2i_loss = loss(mbankT_t2i_sim, ground_truth)
                mbankT_ori_loss = (mbankT_i2t_loss + mbankT_t2i_loss)/2
        ...
        with autocast():
            ...
            batch_i2t_loss_2 = loss(batch_i2t_sim2, ground_truth)
            batch_t2i_loss_2 = loss(batch_t2i_sim2, ground_truth)

            batch_i2t_loss = loss(batch_i2t_sim, ground_truth)
            batch_t2i_loss = loss(batch_t2i_sim, ground_truth)

        batch_prob = prob[ori_idx]
        batch_prob = batch_prob.to(batch_i2t_loss.device)
        total_loss = torch.mean(batch_prob * (batch_i2t_loss_2 + batch_t2i_loss_2)/2)

        batch_i2t_ori_loss = batch_prob * batch_i2t_loss
        batch_t2i_ori_loss = batch_prob * batch_t2i_loss

        # batch_i2t_ori_loss = batch_i2t_loss
        # batch_t2i_ori_loss = batch_t2i_loss
        ...
        with torch.no_grad():
            ...
            with autocast():
                ...
                mbank_i2t_new_loss = loss(mbankI_i2t_sim_new, ground_truth)
                mbank_t2i_new_loss = loss(mbankI_t2i_sim_new, ground_truth)
                mbank_img_new_loss = (mbank_i2t_new_loss + mbank_t2i_new_loss)/2

                mbankT_i2t_new_loss = loss(mbankT_i2t_sim_new, ground_truth)
                mbankT_t2i_new_loss = loss(mbankT_t2i_sim_new, ground_truth)
                mbank_txt_new_loss = (mbankT_i2t_new_loss + mbankT_t2i_new_loss)/2
        ...
        if (idx % args.display == 0) and (idx != 0):
            logger.info("Epoch: %d/%d, step:%d/%d, lr: %.8f, sloss: %f",
