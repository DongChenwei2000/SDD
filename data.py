...
class LoadDataset(Dataset):
    def __init__(self, args, dataset_root, preprocess, subset=None, logger=None, split=None):
            ...
            if self.noise_ratio != 0:
                ...
                for line in open(os.path.join(dataset_root, 'annotations/scan_split/train_ids.txt'), 'r'):
                    for i in range(self.num_anns):
                        if self.dataset == 'cc':
                            self.all_images_id.append(str(line.strip()))
                        else:
                            self.all_images_id.append(int(line.strip())) # "int(line.strip())" for MSCOCO and Flickr30K; "str(line.strip())" for CC120K
                ...
                for line in open(os.path.join(dataset_root, 'annotations/scan_split/{}_noise_train_caps.txt' .format(self.noise_ratio)), 'r', encoding='utf-8'):
                        self.all_captions.append(line.strip())
                logger.info('%d captions have been loaded.', len(self.all_captions))
                ...
                self.mbank_i = np.load(os.path.join('/home/user8/dcw/code/SDD/dataset/coco/', 'annotations/query_bank/{}_mbank_img_idx.npy' .format(self.noise_ratio)), allow_pickle=True).item()
                self.mbank_t = np.load(os.path.join('/home/user8/dcw/code/SDD/dataset/coco/', 'annotations/query_bank/{}_mbank_txt_idx.npy' .format(self.noise_ratio)), allow_pickle=True).item()

                # get images' name list
                self.all_image_name = id2name(self.all_images_id, self.dataset,'train', self.dataset_root)

                self.images_id = [self.all_images_id[i] for i in split]
                self.image_name = [self.all_image_name[i] for i in split]
                self.captions = [self.all_captions[i] for i in split]
                self.ori_idx = [i for i in split]

                # tokenize annotation captions
                self.cap_token = clip.clip.tokenize(self.captions)
                self.all_cap_token = clip.clip.tokenize(self.all_captions)
                self.img_length = len(set(self.images_id))
                self.cap_length = len(self.captions)
                logger.info('%d images have been loaded.', self.img_length)
                logger.info("%s set initialization completed!", subset)
                ...
                for line in open(os.path.join(dataset_root, 'annotations/scan_split/%s_ids.txt' % subset), 'r'):
                    for i in range(self.num_anns):
                        if self.dataset == 'cc':
                            ...
                            self.all_images_id.append(int(line.strip())) # "int(line.strip())" for MSCOCO and Flickr30K; "str(line.strip())" for CC120K
                logger.info('%d images have been loaded.', len(set(self.all_images_id)))
                # get annotation sentences
                for line in open(os.path.join(dataset_root, 'annotations/scan_split/%s_caps.txt' % subset), 'r', encoding='utf-8'):
                        self.all_captions.append(line.strip())
                logger.info('%d captions have been loaded.', len(self.all_captions))
    ...
    def __getitem__(self, idx):
        if self.subset == 'train':
            ...
            mbank_img = self.preprocess(Image.open(os.path.join(self.image_root, self.all_image_name[mbank_i_idx])))
            mbank_i2txt = self.all_cap_token[mbank_i_idx]

            mbank_t_idx = int(self.mbank_t[str(int(idx))])
            mbank_txt = self.all_cap_token[mbank_t_idx]
            mbank_t2img = self.preprocess(Image.open(os.path.join(self.image_root, self.all_image_name[mbank_t_idx])))
            
            ori_idx = self.ori_idx[idx]

            return image, text, idx, img_id, mbank_img, mbank_i2txt, mbank_txt, mbank_t2img, ori_idx
        ...
        elif self.subset == 'mb_bank':
            image = self.preprocess(Image.open(os.path.join(self.image_root, self.image_name[idx]))) # Image from PIL module
            text = self.cap_token[idx]
            img_id = self.images_id[idx]
            return image, img_id, text, idx     
...
def prepare_dataloader(args, dataset_root, preprocess, logger, subset, split=None):
    dataloders = {}
    data = LoadDataset(args, dataset_root, preprocess, subset=subset, logger=logger, split=split)
